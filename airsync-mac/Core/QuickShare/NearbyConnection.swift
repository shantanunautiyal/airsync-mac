...
public class NearbyConnection{
	...
	private func receiveFrameAsync(length:UInt32){
		connection.receive(minimumIncompleteLength: Int(length), maximumLength: Int(length)) { [weak self] content, contentContext, isComplete, error in
			Task { @MainActor in
				guard let self=self else { return }
				if self.connectionClosed{
					return
	...
	internal func disconnect(){
		connection.send(content: nil, isComplete: true, completion: .contentProcessed({ [weak self] error in
			Task { @MainActor in
				self?.handleConnectionClosure()
			}
		}))
