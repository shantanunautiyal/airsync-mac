...
class AppState: ObservableObject {
    ...
    func checkLicense() async {
        ...
        guard let key = self.licenseDetails?.key, !key.isEmpty else {
            self.isPlus = false
            incrementInvalidLicenseFailCount() // treat as invalid (no key)
            UserDefaults.standard.lastLicenseCheckDate = now
            return
        }

        do {
            let valid = try await checkLicenseKeyValidity(
                key: key,
                save: false,
            ...
            if valid {
                ...
                self.isPlus = false
                incrementInvalidLicenseFailCount()
            ...
            if consecutiveDays >= 3 {
                ...
                performUnregisterWithAlert(reason: "Could not validate your license for 3 consecutive days due to network issues. Please re-enter your key when you’re online.")
            } else {
            ...
            if UserDefaults.standard.consecutiveNetworkFailureDays >= 3 {
                performUnregisterWithAlert(reason: "Could not validate your license for 3 consecutive days due to server issues. Please re-enter your key when you’re online.")
            } else {
    ...
    private func incrementInvalidLicenseFailCount() {
    ...
    private func performUnregisterWithAlert(reason: String) {
